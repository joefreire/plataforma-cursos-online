...
<div class="row">
	<div class="col-lg-3 col-sm-4">
		...
		<div class="card card-block">
			<div class="thumb thumb-rounded" style="width: 100px;">
				<img @if(Auth::user()->foto == '') src="/assets/img/nopicture.png" @else src="/uploads/usuarios/{{ Auth::user()->foto }}" @endif style="width:100px; height:90px;">
			</div>
			<div class="caption text-center">
				<h3 class="m-t-20">{{ Auth::user()->name }} </h3>
				<p class="mutted">{{ Auth::user()->email }} </p>
				<button onclick="$('#file_input').click()" type="button" class="btn btn-info">Alterar Foto do Perfil</button>
				<form id="form_foto_{{ Auth::user()->id }}" method="POST" action="/Instrutor/{{ Auth::user()->id }}/foto" enctype="multipart/form-data">
					{{ csrf_field() }}
	...
	<div class="col-lg-9 col-sm-8">
		...
		<div class="card card-inverse card-flat">
			...
			<form id="form-dados" action="/Instrutor/Atualiza" method="POST">
				<table id="dados" class="table table-borderless table-striped">
					<tbody>
						...
						<tr>
							...
							<td colspan="2">
								<input id="cpf" class="form-control" type="text" onkeypress="return SomenteNumero(event);" name="cpf" value="{{ Auth::user()->cpf }}">
							</td>
						</tr>
						<tr>
							<td><strong>Telefone</strong></td>
							<td colspan="2">
								<input class="form-control" type="text" onkeypress="return SomenteNumero(event);" name="telefone" value="{{ Auth::user()->telefone }}">
							</td>
						</tr>
						<tr>
							<td><strong>CEP</strong></td>
							<td colspan="2">
								<input id="cep" class="form-control" onkeypress="return SomenteNumero(event);" type="text" name="cep" value="{{ Auth::user()->cep }}">
							</td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td colspan="2">
								<input id="uf" class="form-control" type="text" name="estado" maxlength="2" value="{{ Auth::user()->estado }}">
						...
						<tr>
							...
							<td>
								<select class="form-control" name="bankNumber" id="bankNumber" style="width: 100%;" disabled="disabled">
									...
									<option value="{{$banco->cod}}" {{( Auth::user()->bankNumber == $banco->cod ? "selected" : '')}}>{{$banco->banco}}</option>
									@endforeach
								</select>
							</td>
						</tr>
						<tr>
							...
							<td>
								<input id="agencyNumber" class="form-control" type="text" onkeypress="return SomenteNumero(event);" name="agencyNumber" placeholder="Número da agência" value="{{ Auth::user()->agencyNumber }}">
							</td>
							<td>
								<input id="agencyCheckNumber" class="form-control" type="text" onkeypress="return SomenteNumero(event);" name="agencyCheckNumber" maxlength="1" placeholder="Digito Verificador" value="{{ Auth::user()->agencyCheckNumber }}">
							</td>
						</tr>
						...
						<tr>
							...
							<td>
								<input id="accountNumber" class="form-control" type="text" onkeypress="return SomenteNumero(event);" name="accountNumber" placeholder="Número da conta" value="{{ Auth::user()->accountNumber }}">
							</td>
							<td>
								<input id="accountCheckNumber" class="form-control" type="text" onkeypress="return SomenteNumero(event);" maxlength="1" name="accountCheckNumber" placeholder="Digito Verificador" value="{{ Auth::user()->accountCheckNumber }}">
							</td>
						</tr>
...
<script>
	...
	$( document ).ready(function() {
		...
		$( "#editar" ).click(function() {			
			$('#form-dados input, #form-dados textarea, #form-dados select, #form-dados button').attr('disabled',false);
